...
class AttributionEngine:
    ...
    def _get_snapshot(self, block_number: int, vhk: str) -> Optional[BlockSnapshots]:
        stmt = (
    ...
    def _get_yield(self, block_number: int, vhk: str) -> Optional[BlockYields]:
        stmt = (
            select(BlockYields)
            .where(
                and_(
    ...
    def _get_attributions_for_block(self, block_number: int, vhk: str):
        stmt = (
    ...
    def _get_attributions_range(
        self, start: int, end: int, vhk: Optional[str] = None, subnet_id: Optional[int] = None
    ):
        conditions = [
    ...
    def _record_gap(
        ...
        for gap in existing:
            if (
                gap.validator_hotkey == vhk
                and gap.resolution_status == ResolutionStatus.OPEN.value
            ):
    ...
    def _merge_overlapping_gaps(
        self, gap_type: GapType, vhk: Optional[str] = None
    ) -> int:
        conditions = [
        ...
        stmt = select(DataGaps).where(and_(*conditions)).order_by(DataGaps.block_start)
        gaps = list(self.session.scalars(stmt).all())
        if len(gaps) <= 1:
            return len(gaps)
    ...
    def consolidate_gaps(self, vhk: Optional[str] = None) -> dict:
        results = {}
        for gt in GapType:
    ...
    def run_attribution(
        ...
        run = self._create_run(RunType.ATTRIBUTION, validator_hotkey, (start_block, end_block))
    ...
    def _attribute_block(
        ...
        snapshot = self._get_snapshot(block_number, vhk)
        if not snapshot:
            if not dry_run:
                self._record_gap(
                    GapType.SNAPSHOT, block_number, block_number, vhk,
                    "Missing snapshot during attribution", run_id,
                )
            return None

        block_yield = self._get_yield(block_number, vhk)
        if not block_yield:
            if not dry_run:
                self._record_gap(
                    GapType.YIELD, block_number, block_number, vhk,
                    "Missing yield during attribution", run_id,
                )
            return None

        yield_earned = Decimal(str(block_yield.total_dtao_earned))
        if yield_earned == 0:
        ...
        total_proportion = sum(Decimal(str(d.proportion)) for d in snapshot.delegations)
        ...
        snap_flag = CompletenessFlag(snapshot.completeness_flag)
        yield_flag = CompletenessFlag(block_yield.completeness_flag)
        completeness = self._compute_completeness(snap_flag, yield_flag)

        attributions: list[BlockAttributions] = []
        total_attributed = Decimal(0)

        for d in snapshot.delegations:
            proportion = Decimal(str(d.proportion))
            attributed_dtao = (yield_earned * proportion).quantize(
        ...
        remainder = yield_earned - total_attributed
        if remainder > 0 and attributions:
            largest = max(attributions, key=lambda a: a.attributed_dtao)
            largest.attributed_dtao = float(
                Decimal(str(largest.attributed_dtao)) + remainder
            )
    ...
    def list_attributions(
        ...
        rows = self._get_attributions_range(start, end, validator_hotkey, subnet_id)
        return [
            {
                "id": r.id,
    ...
    def get_stats(
        self, start: int = 0, end: int = 0, validator_hotkey: Optional[str] = None
    ) -> dict:
        """Return stats suitable for AttributionStatsResponse."""
        rows = self._get_attributions_range(start, end, validator_hotkey)
        if not rows:
            return {
    ...
    def get_block_detail(
        self, block_number: int, validator_hotkey: Optional[str] = None
    ) -> Optional[dict]:
        """Return block detail suitable for BlockDetailResponse."""
        conditions = [BlockAttributions.block_number == block_number]
        if validator_hotkey:
        ...
        vhk = rows[0].validator_hotkey
        snapshot = self._get_snapshot(block_number, vhk)
        total_dtao = sum(Decimal(str(r.attributed_dtao)) for r in rows)
    ...
    def get_attribution_stats(
        ...
        rows = self._get_attributions_range(start_block, end_block, validator_hotkey)
        if not rows:
            return {
        ...
        for a in rows:
            ...
            dtao = Decimal(str(a.attributed_dtao))
            total_dtao += dtao
    ...
    def validate_attributions(
        ...
        results = {"blocks_checked": 0, "blocks_valid": 0, "blocks_invalid": 0, "issues": []}
        for bn in range(start_block, end_block + 1):
            snapshot = self._get_snapshot(bn, validator_hotkey)
            block_yield = self._get_yield(bn, validator_hotkey)
            attribs = self._get_attributions_for_block(bn, validator_hotkey)
            ...
            if len(attribs) != len(snapshot.delegations):
                ...
                results["issues"].append(
                    {"block": bn, "issue": "count_mismatch", "expected": len(snapshot.delegations), "actual": len(attribs)}
                )
            total = sum(Decimal(str(a.attributed_dtao)) for a in attribs)
            if total != Decimal(str(block_yield.total_dtao_earned)):
                valid = False
                results["issues"].append(
                    {"block": bn, "issue": "total_mismatch", "expected": str(block_yield.total_dtao_earned), "actual": str(total)}
                )
            if valid:
