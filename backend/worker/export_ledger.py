...
Usage:
    ...
    python -m worker.export_ledger --period-type MONTHLY --start 2026-01-01 --end 2026-01-31 -o report.csv
"""
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Export ledger entries to CSV")
    parser.add_argument(
        "--period-type", "-t", required=True,
        choices=[e.value for e in PeriodType],
        help="Period type (DAILY or MONTHLY)",
    )
    parser.add_argument(
        "--start", "-s", required=True, type=date.fromisoformat,
        help="Period start (YYYY-MM-DD)",
    )
    parser.add_argument(
        "--end", "-e", required=True, type=date.fromisoformat,
    ...
    parser.add_argument(
        "--include-incomplete", action="store_true", default=True,
        help="Include entries with incomplete data (default: true)",
    )
    args = parser.parse_args(argv)

    period_type = PeriodType(args.period_type)
